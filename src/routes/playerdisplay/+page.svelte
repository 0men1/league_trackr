...
<script>
	...
	// @ts-nocheck
	import { check_account_validity, get_game_info, get_current_game_info, get_champ_image, get_spell_image, get_acc_opgg, get_acc_ugg, ingame} from "./league_api";
	...
	let id = ''; //user's id
	let user = ''; // user's summoner name
	let region = ''; // user's region
	let storedData; // stored data from local storage
			...
			<ul class='item inline w-fit h-full text-white list'>
				...
				<li class=' list-none item px-4 h-fit border-2 border-black flex items-center justify-between gap-6'>
					...
					<div class='flex gap-2'>
						<a target="_blank" 
						draggable="false" 
						href={get_acc_opgg(player.name,storedData.region)} 
						id='opgg' class='p-2 border-black border-2 rounded-lg hover:cursor-pointer flex justify-center'>OP.GG</a>


						<a target="_blank" 
						draggable="false" 
						href={get_acc_ugg(player.name,storedData.region)} 
						id='ugg' 
						class='p-2 border-black border-2 rounded-lg hover:cursor-pointer justify-center flex'>U.GG</a>
